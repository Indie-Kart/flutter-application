import '../cattwo_screen/widgets/productgrid1_item_widget.dart';
import 'bloc/cattwo_bloc.dart';
import 'models/cattwo_model.dart';
...
class CattwoScreen extends StatelessWidget {
  CattwoScreen({Key? key}) : super(key: key);

  GlobalKey<NavigatorState> navigatorKey = GlobalKey();

  static Widget builder(BuildContext context) {
    return BlocProvider<CattwoBloc>(
        create: (context) =>
            CattwoBloc(CattwoState(cattwoModelObj: CattwoModel()))
              ..add(CattwoInitialEvent()),
        child: CattwoScreen());
  }
    ...
    return CustomAppBar(
        ...
        title: Container(
            ...
            child: Stack(alignment: Alignment.centerLeft, children: [
              ...
              AppbarSubtitleOne(
                  text: "lbl_fashion".tr,
                  margin: EdgeInsets.fromLTRB(24.h, 4.v, 159.h, 5.v))
            ])));
  }
  ...
  Widget _buildFilterSection(BuildContext context) {
    return Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Padding(
              padding: EdgeInsets.only(left: 10.h),
              child: BlocSelector<CattwoBloc, CattwoState, CattwoModel?>(
                  selector: (state) => state.cattwoModelObj,
                  builder: (context, cattwoModelObj) {
                    return CustomDropDown(
                        ...
                        items: cattwoModelObj?.dropdownItemList ?? [],
                        onChanged: (value) {
                          context
                              .read<CattwoBloc>()
        ...
        child: BlocSelector<CattwoBloc, CattwoState, CattwoModel?>(
            ...
            builder: (context, cattwoModelObj) {
              return GridView.builder(
                  ...
                  itemCount: cattwoModelObj?.productgrid1ItemList.length ?? 0,
                  itemBuilder: (context, index) {
                    Productgrid1ItemModel model =
                        cattwoModelObj?.productgrid1ItemList[index] ??
                            Productgrid1ItemModel();
                    return Productgrid1ItemWidget(model);
                  });
            }));
  }
